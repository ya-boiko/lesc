...
from app.telemetry.traces import span_context_links

from .unit_of_work import AbstractUnitOfWork


logger = logging.getLogger(__name__)
tracer = trace.get_tracer("bus")
...
def span_links() -> List[trace.Link] | None:
    """Provide links to the span context."""
    links = span_context_links.get()

    # Forget the context so that the derived messages don't inherit it
    span_context_links.set(None)
...
class MessageBus(AbstractMessageBus):
    ...
    def handle(self, message: Message) -> List[Any]:
        ...
        with tracer.start_as_current_span(links=span_links(), **self._trace_span_info(message)):
    ...
    def _handle_event_with_handler(self, event: events.Event, handler: Callable) -> HandlerResult:
        with tracer.start_as_current_span(f"{handler.__name__}"):
    ...
    def _handle_unknown(self, message):
        ...
        with tracer.start_as_current_span(f"UNKNOWN {message_cls_name}", attributes={"message": repr(message)}):
            raise Exception(f"{message} was not an Event or Command")
